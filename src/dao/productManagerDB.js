  ...
  async createProduct(productData) {
    try {
      if (Array.isArray(productData)) {
        const result = await productsModel.insertMany(productData); // No me deja insertar varios productos simultanemente
        console.log(result);
        return result;
      } else {
        const newProduct = new productsModel(productData);
        const result = await newProduct.save(productData);
        return result;
      }
    } catch (error) {
